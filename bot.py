...
def btn(text, callback_data=None, url=None, copy_text=None, style=None):
    """Create button with style support"""
    extra = {}
    if style:
        extra["style"] = style
    
    if url:
        return types.InlineKeyboardButton(text=text, url=url, **extra)
    elif copy_text:
        return types.InlineKeyboardButton(text=text, copy_text=types.CopyTextButton(text=copy_text), **extra)
    else:
        return types.InlineKeyboardButton(text=text, callback_data=callback_data, **extra)
...
def price_command(message):
    ...
    kb.add(btn("🔙 BACK", callback_data="back_profile", style="danger"))
    ...
    for attempt in range(max_retries):
        try:
            ...
            if data.get("meta", {}).get("code") == 200:
                ...
                threading.Thread(
                    target=auto_check_otp, args=(chat_id, full_num, status_id), daemon=True
                ).start()
                return
...
def handle_text(message):
    ...
    if txt == "📱 NUMBER BUY":
        ...
        markup.add(btn("🔙 BACK", callback_data="back_profile", style="danger"))
        msg_text = (
            "╔════════════════════╗\n"
            "      👤 𝚄𝚂𝙴𝚁 𝙿𝚁𝙾𝙵𝙸𝙻𝙴\n"
            "╚════════════════════╝\n"
...
def handle_query(call):
    ...
    if call.data == "noop":
        ...
        markup.add(btn("❌ CANCEL", callback_data="cancel_withdraw", style="danger"))
        bot.edit_message_text(
        ...
        try:
            ...
            bot.send_message(cid, "❌ Withdrawal Cancelled")
