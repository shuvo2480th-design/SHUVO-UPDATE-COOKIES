...
def service_menu_markup():
    """GET NUMBER → Facebook WhatsApp Telegram Instagram বাটন দেখাবে সবসময়"""
    rows = []
    for svc in FIXED_SERVICES:
        icon = SERVICE_ICONS.get(svc, "📱")
        rows.append([make_button(f"{icon} {svc.upper()}", callback_data=f"sv_{svc}", style="primary")])
...
def auto_check_otp_nexus(chat_id, phone_numbers, api_ids, search_msg_id=None):
    """Nexus OTP check — nexus_.txt থেকে হুবহু"""
    ...
    try:
        while True:
            if time.time() - start_time > 1800:
                otp_running[chat_id] = False
                try: bot.send_message(chat_id, "⏰ Time Expired!")
                except Exception: pass
                return
            current_nums = user_numbers.get(chat_id, [])
            if not isinstance(current_nums, list):
                current_nums = [current_nums] if current_nums else []
            if not current_nums or user_panel.get(chat_id) != "nexus":
                otp_running[chat_id] = False; return

            try:
                ids_to_check = user_number_ids.get(chat_id, api_ids)
                if not ids_to_check: time.sleep(2); continue
                ...
                for item in all_items:
                    msg_id  = item.get("otp_id") or item.get("id")
                    api_num = clean_number(item.get("number", ""))
                    if msg_id in global_used_otps.get(chat_id, set()): continue
...
def handle_query(call):
    ...
    # ========== NORMAL CALLBACKS ==========
        ...
        rows = []
        # stex দেশ
        stex_countries = service_countries.get("stex", {}).get(service_name, [])
        for idx, country in enumerate(stex_countries):
            flag  = get_flag(country["name"])
            label = f"📡 {flag} {country['name']}"
            rows.append([make_button(label, callback_data=f"ct_stex__{service_name}__{idx}", style="success")])
        # nexus দেশ
        nexus_countries = service_countries.get("nexus", {}).get(service_name, [])
        for idx, country in enumerate(nexus_countries):
            flag  = get_flag(country["name"])
            label = f"🔗 {flag} {country['name']}"
            rows.append([make_button(label, callback_data=f"ct_nexus__{service_name}__{idx}", style="primary")])
        if not rows:
            rows.append([make_button("⚠️ কোনো দেশ এড হয়নি", callback_data="noop", style="danger")])
