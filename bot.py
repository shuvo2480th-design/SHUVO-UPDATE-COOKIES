...
def otp_result_markup(otp, price=None):
    ...
    rows.append([
        make_button("🔄 Change Number", callback_data="change_number", style="primary"),
        make_button("🔐 OTP GROUP", callback_data="otp_group", style="info"),
    ])
    rows.append([make_button("🔙 BACK", callback_data="back_to_main", style="danger")])
    if price is not None:
        rows.append([make_button(f"Add {price:.2f} Tk", callback_data="add_money", style="success")])
    return build_inline_keyboard(rows)
