...
# ===================== কনফিগারেশন =====================
API_KEY      = "MUBTR1MKUBO"
BOT_TOKEN    = "8738544813:AAFPIv_GVioHShWCbYzuIQeqB41d-pAvFOA"
...
def make_box(lines):
    """একাধিক লাইনকে সুন্দর করে <pre> বক্সে সাজায় (Telegram-এ মনোস্পেস, তাই লাইন ভাঙে না)"""
...
def otp_result_markup(otp, price=None):
    ...
    if price is not None:
        rows.append([make_button(f"💰 ADD BALANCE {price:.2f} ৳", callback_data="noop", style="success")])
    return build_inline_keyboard(rows)
            ...
            if panel == "nexus":
                ...
                try:
                    for idx, api_id in enumerate(api_ids):
                        ...
                        box = make_box([f"{display_flag} {matched_num}    {tag}"])
                        text = f"{box}\n💸 𝐄𝐚𝐫𝐧𝐞𝐝 : {current_price:.2f} ৳\n✅ 𝐒𝐭𝐚𝐭𝐮𝐬 : 𝐒𝐮𝐜𝐜𝐞𝐬𝐬"
                        kb = otp_result_markup(otp, price=current_price)
                        if not first_otp_found and search_msg_id:
                            try:
                                bot.edit_message_text(text, chat_id, search_msg_id, reply_markup=kb, parse_mode="HTML")
...
def process_number(message, edit_msg=None, service_name="Unknown", rid=None):
    ...
    loading_text = make_box(["🔄 GETTING NUMBER...", "⏳ PLEASE WAIT..."])
    if edit_msg:
        try:
            bot.edit_message_text(loading_text, chat_id, edit_msg.message_id, parse_mode="HTML")
            status_id = edit_msg.message_id
        except Exception:
            status_id = bot.send_message(chat_id, loading_text, parse_mode="HTML").message_id
    else:
        status_id = bot.send_message(chat_id, loading_text, parse_mode="HTML").message_id
    ...
    # ✅ ফ্ল্যাগ + নাম্বার + সার্ভিস ট্যাগ বক্স আকারে দেখানো হচ্ছে
    tag = get_service_tag(service_name)
    box_lines = [f"{get_flag(countries[i] if i < len(countries) else '')} {num}    {tag}" for i, num in enumerate(nums)]
    msg_text = make_box(box_lines)
    kb = build_inline_keyboard(
        # নাম্বার বাটনগুলো
    ...
    try:
        bot.edit_message_text(msg_text, chat_id, status_id, reply_markup=kb, parse_mode="HTML")
    except Exception:
